...
EEPROM 記憶體位置分配： 位址："0"為儲存UID的筆數。 位址："1" ：為標註主卡已經被儲存記號 。

設定順序：
IO PORT初始化-->串列初始化-->MFRC522初始化-->設定天線增易--> 檢查EEPROM記憶體是否需要清除？需按住按鍵15sec以上-->檢查是否有主卡資料
如果沒有則進入設定模式-->紀錄第一筆主卡資料方能進入正常模式。

getID()函數是負責讀取卡片UID資料-->放到readCard 矩陣-->isMaster() 將EEPROM的 masterCard矩陣比對傳回 "1" 真或"0"假

經過實驗悠遊卡，一卡通也可以使用『2018-11-06』
...
#define relay  2	//4			// Set Relay Pin
...
//        }
//        else {
          EEPROM.update(x, 0); 			// if not write 0 to clear, it takes 3.3mS
//        }
      }
...
void loop () {
  do {
    ...
    if (programMode) {
      cycleLeds();              // 藍色LED ON Program Mode cycles through RGB waiting to read a new card 程序模式循環通過RGB等待讀取新卡
    }
    else {
      normalModeOn(); 		// 綠色 LED ON  Normal mode, blue Power LED is on, all others are off正常模式下，綠色電源LED亮起，其他所有關閉
  ...
  // 在燒錄模式
  // 1. 尋找EEPROM 如果有符合的UID 就刪除
  // 2. 尋找EEPROM 如果沒有符合的UID 就新增
  if (programMode) {
    ...
    while (mfrc522.PICC_IsNewCardPresent())	//是否感應到卡片【卡片移除否?】
    {
      digitalWrite(relay,LOW);
...
//	Serial.println("IsNewcard 0 return");
    return 0;
  }
  if ( ! mfrc522.PICC_ReadCardSerial()) {   //Since a PICC placed get Serial and continue
//		Serial.println("ReadCardSerial 0 return");
