...
void setup()
{
  ...
  Serial.begin(115200);
  Serial.println("VR20231223_OLED_LOG.ino");
...
void loop()
{
  ...
  if (currentMillis - previousMillis >= interval)
  {
    ...
    if (digitalRead(P_key) == Press)
    {
      ...
      Serial.println(SS_VOLTAGE + "," + SS_MIDI + "," + DEC_MIDI);  //輸出CSV檔案格式好方便將log file匯入excel分析
      //        }

      while (digitalRead(P_key) == Press); // wiat key reless
      delay(debounce_delay);  //debounce time
      //      }
    }
    workCounter--;
