...
#define vref_val 3.3231 // 開發板的參考電壓使用SANWA PC7000三用表量測
#define SerialVersion "VR20240109_ATSAMD21D_LCD_LOG_SD.ino"
...
void real_vr(void)
{
  ...
  if (analogValue != old_analogValue)
  {
    ...
    u8g2.print(" DEC:");         // 顯示MIDI值
    u8g2.print(shiftvalue, DEC); // 顯示MIDI值
    ...
    //  SerialUSB.println("VR MIDI Value:" + String(MIDI));
    ...
    SerialUSB.print(testCounter);
    SerialUSB.print(",");
    SerialUSB.print(voltage, 4); // 送出量測電壓值
    SerialUSB.print(",");
    SerialUSB.print(shiftvalue, HEX); // 送出16進制MIDI
    SerialUSB.print(",");             // SS_VOLTAGE + "," + SS_MIDI );
    SerialUSB.print(shiftvalue, DEC); // 送出10進制MIDI
    SerialUSB.print(",");
    SerialUSB.println(absvalu); // 送出與中心電壓MIDI值的差距絕對值
                                //  SerialUSB.println("," + Result); // 輸出CSV檔案格式好方便將log file匯入excel分析
  }
}
void vr_000(void)
{
...
void vr_100(void)
{
  ...
  voltage = analogValue;
  old_analogValue = analogValue;
  //        SerialUSB.println("ADC:" + String(voltage));
  int shiftvalue = analogValue >> 3; // 右移3bit
  absvalu = abs(64 - shiftvalue);
  if (absvalu <= guige) //2024/02/23 規格放寬為由3-->4
  {
    Result = "OK";
  }
  else
  ...
  SerialUSB.print(now.year());
  SerialUSB.print("/");
  SerialUSB.print(now.month());
  SerialUSB.print("/");
  SerialUSB.print(now.day());
  SerialUSB.print(",");
  SerialUSB.print(now.hour());
  SerialUSB.print(":");
  SerialUSB.print(now.minute());
  SerialUSB.print(":");
  ...
  SerialUSB.print(shiftvalue, DEC); // 送出10進制MIDI
  SerialUSB.print(",");
  SerialUSB.print(absvalu);        // 送出與中心電壓MIDI值的差距絕對值
  SerialUSB.println("," + Result); // 輸出CSV檔案格式好方便將log file匯入excel分析
                                   // SD 卡資料寫入
...
void setup()
{
  ...
  SerialUSB.println(SerialVersion);
  ...
  if (!SD.begin(CS))
  {
    SerialUSB.println("SD卡初始化失敗");
    u8g2.setCursor(0, 55);
    u8g2.print("SD int NG！！"); // 顯示電壓值

    return;
  }
  SerialUSB.println("SD卡初始化成功");

  // 打開或建立一個名為data.csv的文件
  dataFile = SD.open("vrdata.csv", FILE_WRITE);
  if (dataFile)
  {
    SerialUSB.println("文件打開成功");
    u8g2.setCursor(0, 55);
    u8g2.print("SD Op OK！！"); // 顯示電壓值
  }
  else
  {
    SerialUSB.println("文件打開失敗");
    u8g2.setCursor(0, 55);
    u8g2.print("SD Op NG！！"); // 顯示電壓值
