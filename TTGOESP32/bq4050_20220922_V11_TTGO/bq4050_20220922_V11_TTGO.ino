...
#define mode_range 4 // Mode 的設定上限
//#define u8g2_font_unifont_t_chinese1  u8g2_font_unifont_t_chinese1
//#define u8g2_font_osb29_tf  u8g2_font_osb29_tf
int KEY_CHEN;
int KEY_REP;
bool Blink1HZ;
bool Blink2HZ;
...
//  tft.fillScreen(TFT_BLACK);
 // tft.init();
  ...
  u8f.setFont(u8g2_font_unifont_kaishu7216);	//t_chinese1); // use chinese2 for all the glyphs of "你好世界"
//  if (F_changeColer1 == 0)
//  {
    u8f.setForegroundColor(TFT_WHITE); // apply color
    u8f.setCursor(0, 15);
    u8f.print("亞弘電科技生技課 V11");
...
//  pa = registers.relativeStateOfCharge.val;
  ...
  else
  {
	...
	u8f.setFont(u8g2_font_unifont_kaishu7224);	//t_chinese1); // use chinese2 for all the glyphs of "你好世界"
	u8f.setCursor(20,150);
	u8f.setForegroundColor(TFT_PURPLE); // apply color
	u8f.print("程式設計：李進衛");
  u8f.setCursor(0,190);
	u8f.setForegroundColor(TFT_SKYBLUE); // apply color
	u8f.print("生技課");
...
/*
  ...
  u8f.print(" 電壓");                //
                                     //	u8g2_1.setFont(u8g2_font_courB08_tf); //(u8g2_font_amstrad_cpc_extended_8f);  //(u8g2_font_courB08_tf);  //(u8g2_font_victoriabold8_8u);  //(u8g2_font_fub20_tf); //(u8g2_font_ncenB14_tr); // choose a suitable font
  u8f.print(": ");                   //
  u8f.setFont(u8g2_font_osb29_tf);  //(u8g2_font_wqy15_t_chinese1);  //大字
//    u8f.setForegroundColor(TFT_BLACK); // apply color
    OldV = V;
//   u8f.setCursor(45, 105);
    u8f.print("         ");
//    u8f.setForegroundColor(TFT_WHITE); // apply color
 // u8f.print(V);    //
 // u8f.print("V"); //
...
//    u8f.setForegroundColor(TFT_GREEN); // apply color
  ...
  tft.print(" V"); //
//  BQ4050.readReg(registers.cellVoltage4);
  u8f.setFont(u8g2_font_unifont_kaishu7216); // use chinese2 for all the glyphs of "你好世界"
  u8f.setForegroundColor(TFT_WHITE); // apply color
  u8f.setCursor(100, 85);
  u8f.print("核心四");             //
//  CV4 = registers.cellVoltage4.val;
//  CV4 /= 1000;
...
void Dsp2_200(void)
{
  ...
  u8f.setFontMode(0);                        // use u8g2 none transparent mode
  u8f.setFontDirection(0);                   // left to right (this is default)
  u8f.setFont(u8g2_font_unifont_kaishu7224); // use chinese2 for all the glyphs of "你好世界"
  u8f.setForegroundColor(TFT_RED);           // apply color
  u8f.setCursor(0, 35);
  u8f.print("連線失敗..."); //
  SQN = 3;
...
//  curr /=1000; //換算成安培
  ...
  u8f.setCursor(50,32); //(0, 75);
 // clear_txt();
  u8f.print("電流");
  
	english_gb();
  tft.setCursor(100,30); //(0, 75);
  tft.print(curr,1);
  tft.print("A");
  if (curr > 0.5)
  {
    if (Blink2HZ == 1)
    {
      u8f.setFont(u8g2_font_unifont_kaishu7216);
      u8f.setForegroundColor(TFT_RED); // apply color
        u8f.setCursor(0,50); //(0, 75);
...
void Dsp4_100(void)
{
  ...
  tft.fillScreen(TFT_BLACK);
    u8f.setFont(u8g2_font_osb29_tf);
    u8f.setFontDirection(0);
    u8f.setForegroundColor(TFT_WHITE); // apply color
    u8f.setCursor(0,35);
    u8f.print(Thh);
...
void press(Button2 &btn)
{
  if (btn == buttonA)
  { //按下A按鍵
	keycnt = (keycnt+1)%mode_range; //將累加值限制在指定的範圍內2022-09-22 李進衛更改
...
void betty_300(void)
{
	...
	BQ4050.readReg(registers.cellVoltage2);
	CV2 = registers.cellVoltage1.val;
	CV2 /= 1000;   						//mV-->V
		bt_sqn = 4;
}
...
void betty_400(void)
{
	...
	BQ4050.readReg(registers.cellVoltage3);
	CV3 = registers.cellVoltage1.val;
	CV3 /= 1000;   						//mV-->V
		bt_sqn = 5;
}
...
void Read_bettry_data(void)
{
	
	switch(bt_sqn)
	{
		case 0: betty_000(); break;
		case 1: betty_100(); break;
		case 2: betty_200(); break;
		case 3: betty_300(); break;
		case 4: betty_400(); break;
		case 5: betty_500(); break;
		case 6: betty_600(); break;
		case 7: betty_700(); break;
		case 8: betty_800(); break;
...
void loop()
{
  ...
  if (currentMillis - previousMillis > loopInterval)
  {
    ...
    {
    case 0:
      Display1_proce(); //顯示電量電壓
      break;
    case 1:
      Display2_proce(); //顯示溫度電流
      break;
    case 2:
      Display3_proce(); //顯示核心電壓
      break;
    case 3:
      Display4_proce();    
    }
