...
//LiquidCrystal_I2C lcd(0x27, 20,4); // 1, 0, 4, 5, 6, 7, 3, POSITIVE);  // Set the LCD I2C address
...
void setup()
{
 ...
 //       PORTB7 = LOW;
  ...
  //pinMode(LED, OUTPUT);
    ...
    //****** LCD setting ********
	//*************************
// 2020-05-18 mark  lcd.begin();         // initialize the lcd for 20 chars 4 lines, turn on backlight
  lcd.init();                      // 2020-05-18 lee chin wei initialize the lcd 
  lcd.backlight();		//點亮背光
  // Print a message to the LCD.
  lcd.setCursor(2, 0);	//設定游標在第2列第0行
  lcd.print("YA Horng PE ");
  lcd.setCursor(2, 1);
  lcd.print("MD998 180531");
...
//*******************************************
  ...
  SPI.setClockDivider(SPI_CLOCK_DIV64); //(SPI_CLOCK_DIV64); // SPI clock
  Serial.println(" Program by Lee chin wei");
  Serial.println(" for MD998/969/991/992");
  Serial.println("2018-05-31 append motor torque HI and LOW ");
	delay(1000);
	lcd.clear();
...
void read_key(void)
{
		...
		PORTD_DATA &= 0b01111100; //mark bit0，1，6，7
		
		switch (PORTD_DATA)
		{
			case 0b01111000 : KEY_NEW = 0x01; break;	//play/pause key is press
			case 0b01110100 : KEY_NEW = 0x02; break;	//33/45 key is press
			case 0b01101100 : KEY_NEW = 0x03; break;
			case 0b01011100 : KEY_NEW = 0x04; break;
			case 0b00111100 : KEY_NEW = 0x05; break;
